...
def validate_password_strength(
    ...
    if len(password_string) < min_length or len(password_string) > max_length:
        is_valid = False
        msgs.append(
            f"Password LENGTH must be BETWEEN: {min_length} to {max_length} characters"
        )
    ...
    if special_char and (
        not (re.compile("".join(allowed_special_chars))).search(password_string)
    ):
        is_valid = False
